...
pub fn annual_rate_of_return(log_returns: &Vec<f64>) -> f64 {
  let mean_return: f64 = mean_return(log_returns);
  let periods_per_year: f64 = 252.0; // for daily returns
  (1.0 + mean_return).powf(periods_per_year) - 1.0
}

/// Drawdowns
pub fn drawdowns(returns: &Vec<f64>) -> Vec<f64> {
  let mut drawdowns: Vec<f64> = Vec::new();
  let mut max_return_so_far = returns[0];
  for &r in returns {
...
pub fn mean_return(log_returns: &Vec<f64>) -> f64 {
  let filtered_vec: Vec<&f64> = log_returns.iter().filter(|&&x| x != 0.0).collect();
...
/// Sharpe Ratio
pub fn sharpe_ratio(log_returns: &Vec<f64>) -> f64 {
  let n: f64 = log_returns.len() as f64;
  if n == 0.0 { return 0.0; };

  let mean: f64 = log_returns.iter().sum::<f64>() / n;
  if mean == 0.0 { return 0.0; };

  let variance: f64 = log_returns.iter().map(|&x| (x - mean).powi(2)).sum::<f64>() / n;
...
pub fn sortino_ratio(log_returns: &Vec<f64>) -> f64 {
  let n: f64 = log_returns.len() as f64;
  if n == 0.0 { return 0.0; };
  ...
  let negative_returns: Vec<f64> = log_returns.iter().filter(|&&x| x < 0.0).map(|&x| x.powi(2)).collect();
...
/// Total Return
pub fn total_return(cum_norm_returns: &Vec<f64>) -> f64 {
  cum_norm_returns[cum_norm_returns.len() - 1]
}
